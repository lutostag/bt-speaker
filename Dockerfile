...
RUN apk add --update --no-cache bluez pulseaudio pulseaudio-bluez espeak libffi-dev dbus-dev glib-dev build-base py2-pip python2-dev cairo-dev gobject-introspection-dev alsa-lib-dev alsa-utils
...
WORKDIR /usr/local/include/rtpsbc
RUN make && cp librtpsbc.so /usr/local/bin/librtpsbc.so
